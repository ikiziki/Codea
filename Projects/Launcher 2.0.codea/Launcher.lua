...
function Launcher:touchBegan(touch)
    if touch.pos.y > HEIGHT / 4 then return false end
...
function Launcher:touchEnded(touch)
    if not self.active then return vec2(0, 0) end
    self.current = touch.pos
...
function Launcher:draw(neilActive, previousFlightTime)
    ...
    if lengthSqr > 0 then
        ...
        line(
        self.start.x + direction.x * projectionStart,
        self.start.y + direction.y * projectionStart,
        self.start.x + direction.x * projectionEnd,
        self.start.y + direction.y * projectionEnd
        )
    end
